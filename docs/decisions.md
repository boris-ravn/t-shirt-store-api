...
### 2026-09-03 — CASL's `'manage'` matches every action, including custom ones — `PromoCode`'s manager grant needs an explicit `cannot('apply', ...)`

Found by manual testing, not code review: `validate` is meant to be client-only (`can('apply', 'PromoCode')`), while a manager gets `can('manage', 'PromoCode')` for full CRUD. A manager could still call `validate` and 409 with `cart-empty` — verified against the installed `@casl/ability` directly (`can('manage', X)` then `ability.can('literallyAnyString', X)` returns `true`, `manage` is a real wildcard, not just shorthand for the five CRUD-ish actions this repo happens to use elsewhere). Fixed with an explicit `cannot('apply', 'PromoCode')` after the manager's `can('manage', ...)` — verified that `cannot` after `can` overrides just the one action without weakening `manage` itself. This will recur for any future subject where a custom action (Order's `cancel`/`process`/`ship`/`deliver`) needs to stay off-limits to a role that otherwise holds `manage` on the same subject — check for it explicitly, don't assume `manage` is scoped to obvious CRUD.

### 2026-09-03 — `PromoCode.discountValue`'s range is validated in the service layer, not a schema `CHECK`

Same class of gap as `payments.method` (`database/README.md` §9): `discount_value` means "percent, 1-100" or "minor units" depending on the sibling `discount_type`, and Postgres can express that as a conditional `CHECK`, but this project doesn't yet write conditional constraints by hand for it — validated once at the DTO layer instead (`PercentageDiscountRequestDto.percent` has `@Min(1)@Max(100)`; a `fixedAmount` amount only needs the store-wide non-negative `Money` rule, already enforced). Revisit alongside `payments.method` if a hand-written conditional `CHECK` migration convention gets established there.
