...
> **Superseded 2026-09-06** by "GitHub Actions CI added".
...
### 2026-09-06 — GitHub Actions CI added

`.github/workflows/ci.yml`: two jobs, `checks` (lint, build, unit tests, `lint:openapi`) and `e2e` (gated on `checks` passing), both triggered on push/PR to `main`. The prior entry on this same topic overstated the real dependency: MinIO is dropped entirely — no e2e spec touches S3 (product image upload has no e2e coverage at all, a separately-tracked gap). Postgres is not a `services:` container either — `test/support/e2e-app.ts` already provisions it per-run via `@testcontainers/postgresql`, which needs nothing but a Docker daemon, already present on GitHub-hosted runners. Only Redis and Mailhog are declared as GitHub Actions `services:`, matching `docker-compose.yml`'s images/ports exactly — the one e2e spec that needs them is the low-stock notification smoke test.

No secrets are configured: `test/env-setup.ts` already defaults every env var `env.validation.ts` requires (including placeholder Stripe keys), and the two live-Stripe-API tests self-skip without a real key — both by design already, not something this workflow had to add.

`npm run lint`'s `--fix` flag mutates files, which would let CI silently repair a fixable violation instead of failing on it — CI runs `npx eslint "{src,apps,libs,test}/**/*.ts"` directly instead, without `--fix`.
...
`AuthService.resetPassword`'s `sendPasswordChangedEmail` and `forgotPassword`'s `sendPasswordResetEmail` calls were unguarded — the 2026-08-28 entry above only decided *sync vs. queued*, never what happens on delivery failure. Two distinct bugs followed: `resetPassword`'s password change and refresh-token revocation already commit inside its `$transaction` before the mail call, so an SMTP failure reported an already-successful reset as a 500 with no way for the client to recover. `forgotPassword`'s early-return-on-unknown-email contract (`docs/api/paths/auth.yaml` — "Always answers 202... a 404 here would be a user-enumeration oracle") broke the same way: only the *existing-account* path could 500 on a mail failure, while the unknown-email path always resolved — the exact status-code oracle the early return exists to prevent.

Fixed by wrapping each call in try/catch, logging via `AuthService`'s own `Logger` (matching `StripeWebhookService`'s post-commit best-effort pattern, `src/payments/stripe-webhook.service.ts`), and not rethrowing — both methods now resolve regardless of delivery outcome. Kept inline in `AuthService` rather than centralized in `MailService`, since a service-level swallow would silently change the contract for every existing `MailService` caller without each being reviewed for whether that's safe. BullMQ was considered and rejected again for the same reason as the 2026-08-28 entry — this is about not misreporting an already-committed action, not about needing retryable delivery.
